...
def simulated_annealing(ref_points, max_attempts=1000):
    ...
    while i_score < score_threshold and max_attempts > 0:
        ...
        print('Initial contact points 1:', p_r)
        print('Initial contact points 2:', q_r)

        while temperature > 1e-6 and i_score < score_threshold:
            new_cp1, new_cp2, new_contact_points, new_score = refine_grasp_contact_points(ref_points, cp1, cp2)
            acceptance_prob = acceptance_probability(i_score, new_score, temperature)
...
def predict_grasp_contact_points():
    ...
    # Use simulated_annealing once with initial random contact points
    cp1, cp2, i_score = simulated_annealing(ref_points)
